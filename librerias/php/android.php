					...
					foreach ($filaFirma as $fila ) {
						...
						if($filaProfe){
							foreach ($filaProfe as $filaP ) {
								if(empty($fila['firma'])){
									array_push($prof,array('id'=>$filaP['id'],'nombre'=>$filaP['nombre'],'email'=>$filaP['email'],'firma'=>false));
								}else{
									array_push($prof,array('id'=>$filaP['id'],'nombre'=>$filaP['nombre'],'email'=>$filaP['email'],'firma'=>true));
								}
							}
						}
					}				
